...
def reverse (counter)
	...
	if counter == 0
		puts 'All main tasks have been created!'
		return
	elsif counter % 3 == 0			
		# Spawn some sub-threads, with a message callback
		puts "Main task number " + counter.to_s + " with sub-threads.\n"
		...
		wait = Random.rand(3)
		puts "Main task number " + counter.to_s + " with just a few (" + wait.to_s + ") seconds of sleep.\n"

		sleep wait	# simulate some work...
		reverse (counter-1)
	end	
